...
#Create area plot (NOTE: you will need to change the glm calls if models change!)
```{r}
#library(broom)
library(MASS)
...
lmCW <- glm(count ~ area + EV + OW + WM + Forest400 + 1,
...
for (i in 1:nrow(Dmm)){
  Dmm$ld2[i] = final.data.GLMMPPBGR$localDiversity[i] * final.data.GLMMPPBGR$localDiversity [i]
}
lmMM <- glm.nb(count ~ hydroperiod + EV + OW + Crop400 + Forest400 + localDiversity + ld2 + 1,
...
lmPP <- glm(count ~ area + Crop400 + Forest400 + Grass400 + OW + SM + WM + localDiversity + naturalLandDiversity +
             + alkali + ld2 + 1 + offset(log(size + 1)),
...
sppDat <- rbind(sppDatcw, sppDatpp)
sppDat$data <- factor(sppDat$data, levels = c("CWMP", "DWS"), ordered = TRUE)
...
PBGRplot <- ggplot(data=sppDat, aes(x=area, y=count, color = data, shape = data)) +
  ...
  scale_colour_manual(legend_title, values=c("#a6cee3", "#b2df8a")) +
  scale_shape_manual(legend_title, values = c(1, 5)) +
...
Dcw$area <- as.numeric(Dcw$area)
for (i in 1:nrow(Dcw)){
  Dcw$nld2[i] = final.data.GLCWMPVIRA$naturalLandDiversity[i] * final.data.GLCWMPVIRA$naturalLandDiversity [i]
}
lmCW <- glm(count ~ EV + Crop400 + Forest400 + naturalLandDiversity + nld2 + 1,
            data =  Dcw, family = poisson, na.action = na.pass)
...
Dmm$area <- as.numeric(Dmm$area)
for (i in 1:nrow(Dmm)){
  Dmm$ld2[i] = final.data.GLMMPVIRA$localDiversity[i] * final.data.GLMMPVIRA$localDiversity [i]
}
lmMM <- glm.nb(count ~ hydroperiod + EV + SM + WM + Grass400 + localDiversity + ld2 + 1,
               data =  Dmm, init.theta = 0.1608621864, 
               link = log, na.action = na.pass)
...
Dpp = final.data.PPDWMPPBGR
Dpp$area <- as.numeric(Dpp$area)
lmPP <- glm.nb(count ~ area + alkali + hydroperiod + EV + OW + SM + WM + localDiversity + naturalLandDiversity
                   + 1 + offset(log(size + 1)),
...
sppDat <- rbind(sppDatpp)
sppDat$data <- factor(sppDat$data, levels = c("DWS"), ordered = TRUE)
...
VIRAplot <- ggplot(data=sppDat, aes(x=area, y=count, color = data, shape = data)) +
  ...
  #scale_colour_manual(legend_title, values=c("#a6cee3", "#1f78b4", "#b2df8a")) +
  #scale_shape_manual(legend_title, values = c(1, 2, 5)) +
  scale_colour_manual(legend_title, values=c("#b2df8a")) +
  scale_shape_manual(legend_title, values = c(5)) +
  theme_classic() + #Removes the top and right borders
  theme(panel.grid.minor=element_blank(), panel.grid.major=element_blank()) + #Remove the gridlines
...
Dcw$area <- as.numeric(Dcw$area)
lmCW <- glm(count ~ area + hydroperiod + Crop400 + Forest400 + naturalLandDiversity + 1 + offset(log(size + 1)),
            data =  Dcw, family = poisson, na.action = na.pass)
#create area values over which to predict count
N=1e4
xmin = min(normData[,"area"]); xmax=max(normData[,"area"])
...
Dmm$area <- as.numeric(Dmm$area)
lmMM <- glm.nb(count ~ hydroperiod + EV + SM + WM + Forest400 + localDiversity + 1 + offset(log(size + 1)),
...
Dpp$area <- as.numeric(Dpp$area)
lmPP <- glm.nb(count ~ area + EV + OW + WM + Grass400 + localDiversity + 
                 naturalLandDiversity + 1 + offset(log(size + 1)),
...
lmCW <- glm(count ~ EV + OW + SM + WM + Crop400 + 
              localDiversity + ld2 + naturalLandDiversity + 1,
            data =  Dcw, family = poisson, na.action = na.pass)
...
Dmm$area <- as.numeric(Dmm$area)
for (i in 1:nrow(Dmm)){
  Dmm$nld2[i] = final.data.GLMMPAMBI$localDiversity[i] * final.data.GLMMPAMBI$localDiversity [i]
}
lmMM <- glm(count ~ OW + SM + Crop400 + Grass400 + naturalLandDiversity + nld2 + 1,
...
Dpp$area <- as.numeric(Dpp$area)
for (i in 1:nrow(Dpp)){
  Dpp$ld2[i] = final.data.PPDWMPAMBI$localDiversity[i] * final.data.PPDWMPAMBI$localDiversity [i]
}
lmPP <- glm(count ~ area +  EV + OW + localDiversity + 
              Crop400 + ld2 + alkali + 1 + offset(log(size + 1)),
            data =  Dpp, family = poisson, na.action = na.pass)
...
library(Cairo)
library(gridExtra)
Cairo(file="AreaPlot.png", type="png", units="in", width=10, height=7, pointsize=12, dpi=600)
